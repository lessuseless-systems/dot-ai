---
created: 2025-11-06T23:48:03 (UTC -06:00)
...
---
## [](https://nickel-lang.org/user-manual/package-management/#contracts-in-nickel)Contracts in Nickel
...
For the motivation behind contracts and a high-level overview of contracts and types, first read the [correctness](https://nickel-lang.org/user-manual/correctness) section.
...
**Important**: contracts attached to a record field, such as `{x | Number = 1}`, and a freestanding contract annotation such as `{x = (1 | Number)}` _behave differently with respect to record merging_. However, as far as runtime checks are concerned (which is the focus of this document), they are equivalent.
...
## [](https://nickel-lang.org/user-manual/package-management/#user-defined-contracts)User-defined contracts
...
### [](https://nickel-lang.org/user-manual/package-management/#preamble-idempotency)Preamble: idempotency

As you will see in this section, Nickel contracts are more than mere boolean validators. They can sometimes modify the checked value for valid reasons exposed below. However, it's hard (if not impossible) to rightfully limit the expressive power of contracts to be _reasonable_ with respect to modifications.
...
### [](https://nickel-lang.org/user-manual/package-management/#predicate)Predicate
...
### [](https://nickel-lang.org/user-manual/package-management/#validators)Validators
...
### [](https://nickel-lang.org/user-manual/package-management/#general-custom-contracts)General custom contracts

In some situations, even validators aren't sufficient. For example, when writing [delayed contracts](https://nickel-lang.org/user-manual/package-management/#delayed-contracts) or [parametrized contracts](https://nickel-lang.org/user-manual/package-management/#parametrized-contracts), you might not be able to decide right away if a value satisfies a contract.
...
#### [](https://nickel-lang.org/user-manual/package-management/#label)Label

For immediate checks, signaling failure should be done by returning `'Error`, as for validators, whenever possible. However, for delayed checks, we must do it differently. A custom contract uses `std.contract.blame` which takes the label as an argument. `blame` immediately aborts the execution and reports a contract violation error. You can think of it as throwing an exception, but one that can't be caught.

The label is a special object which is automatically passed to the custom contract by the interpreter. The label contains tracking information for error reporting. Custom contracts can use the label to customize error reporting upon failure using the functions from `std.contract.label`, which set various attributes of the label and return the new modified label. As seen in the [contracts parametrized by contracts section](https://nickel-lang.org/user-manual/package-management/#contracts-parametrized-by-contracts), the label must also be provided when applying a subcontract.

The use of labels is illustrated in the [delayed contracts section](https://nickel-lang.org/user-manual/package-management/#delayed-contracts). Another example of a general custom contracts is given in the next section on parametrized contracts.

### [](https://nickel-lang.org/user-manual/package-management/#parametrized-contracts)Parametrized contracts
...
### [](https://nickel-lang.org/user-manual/package-management/#contracts-parametrized-by-contracts)Contracts parametrized by contracts
...
Parameters that are also contracts may contain delayed checks. In consequence, a contract parametrized by another unknown contract must usually be written as a [custom contract](https://nickel-lang.org/user-manual/package-management/#general-custom-contracts) as well, using the constructor `std.contract.custom`.
...
#### [](https://nickel-lang.org/user-manual/package-management/#apply-vs-check)`apply` vs `check`
...
When implementing a parametrized contract, `apply` should be used for [delayed checks](https://nickel-lang.org/user-manual/package-management/#delayed-contracts). Here is a contrived example re-implementing the builtin contract `[| 'Foo Contract |]` parametrized by `Contract`:
...
#### [](https://nickel-lang.org/user-manual/package-management/#metadata)Metadata

In addition to defining the contracts of fields, record contracts can also attach metadata (see [merging](https://nickel-lang.org/user-manual/merging)) to fields, such as documentation or default values:
...
#### [](https://nickel-lang.org/user-manual/package-management/#open-record-contracts)Open record contracts
...
#### [](https://nickel-lang.org/user-manual/package-management/#giving-values-to-fields)Giving values to fields
...
### [](https://nickel-lang.org/user-manual/package-management/#type-constructors-for-contracts)Type constructors for contracts

We've already seen that the primitive types `Number`, `String` and `Bool` can be used as contracts. In fact, any type constructor of the [static type system](https://nickel-lang.org/user-manual/typing) can be used to combine contracts.

#### [](https://nickel-lang.org/user-manual/package-management/#array)Array
...
A function contract `In -> Out` returns a wrapped function which, for each call, will check the parameter against the `In` contract and the return value against the `Out` contract. Put differently, `In` represents _pre-conditions_ which must hold for the parameter, and `Out` represents _post-conditions_ which must hold for the return value of the function.

##### [](https://nickel-lang.org/user-manual/package-management/#caller-vs-callee)Caller vs callee
...
##### [](https://nickel-lang.org/user-manual/package-management/#higher-order-functions)Higher-order functions
...
#### [](https://nickel-lang.org/user-manual/package-management/#dictionary)Dictionary
...
## [](https://nickel-lang.org/user-manual/package-management/#delayed-contracts)Delayed contracts

This section covers delayed contracts, why they are useful, and how to write them. You might come across the term _lazy contracts_ elsewhere in the documentation, which was the previous denomination of delayed contracts: the two notions are strictly equivalent.

### [](https://nickel-lang.org/user-manual/package-management/#lazy-evaluation)Lazy evaluation

In the [section on writing a custom contract with `std.contract.custom`](https://nickel-lang.org/user-manual/package-management/#general-custom-contracts), we noted the surprising fact that a general custom contract must return `'Ok new_value`, instead of just returning `'Ok`.

The motivation for this return value is laziness. Nickel is designed to be _lazy_, only evaluating values on-demand.

When evaluating a record, Nickel only evaluates the _spine_ of the record, that is the structure of the record (the field names and their metadata), but not its content. The field values are left unevaluated, until they are accessed, printed (in the REPL or as a result of `nickel`), or serialized/exported. Thanks to laziness, you can for example query specific information on a large configuration without having to actually evaluate everything. We will use the always failing contract `std.FailWith` to observe where evaluation takes place:
...
A large configuration usually has a root contract attached, corresponding to its schema. If this contract would perform all the checks immediately, forcing the evaluation of most of the configuration, we would lose the benefits of laziness. Thus, _we want contracts to be lazy as well_. In particular, when writing a contract `{foo | FooContract}`, we want that `FooContract` fires only when the field `foo` is requested. This is indeed the case: built-in contract combinators, such as array contracts, dictionary contracts and function contracts, all have a _delayed part_.

### [](https://nickel-lang.org/user-manual/package-management/#immediate-and-delayed)Immediate and delayed
...
### [](https://nickel-lang.org/user-manual/package-management/#writing-a-delayed-contract)Writing a delayed contract
...
Another use-case for custom delayed contracts is to write a contract that is parametrized by another contract, such as the `Nullable` contract implemented in [the parametrized contracts section](https://nickel-lang.org/user-manual/package-management/#contracts-parametrized-by-contracts). Because the parameter contract is generic and can have a delayed part, `Nullable` needs to be written as a contract with a potential delayed part as well.
...
There is a lot to unwrap here. Please refer to the [syntax section](https://nickel-lang.org/user-manual/syntax) if you have trouble parsing the example.

We first define `with_delayed_checks`, the delayed part, that maps a function over the record which substitutes each field for the same value but wrapped in a `Bool` contract. Thanks to lazyness, the record mapping won't happen until `'Ok with_delayed_checks` is returned _and_ some code actually uses the value. Even then, because records (and record mapping) are lazy, _this doesn't actually execute the `Bool` contracts right away_. Each contract will only be run when the individual fields will be accessed or exported.
...
### [](https://nickel-lang.org/user-manual/package-management/#conclusion)Conclusion
...
## [](https://nickel-lang.org/user-manual/package-management/#boolean-combinators)Boolean combinators
...
It's important to note that those combinators are only _approximations_ of what you could expect from boolean operations. Fortunately, they are overstrict approximation, in that they might reject more values, but will never let invalid values slip through.

The reason is, once again, the presence of delayed contracts.

### [](https://nickel-lang.org/user-manual/package-management/#combining-delayed-contracts)Combining delayed contracts
...
### [](https://nickel-lang.org/user-manual/package-management/#immediate-version-of-built-in-contracts)Immediate version of built-in contracts
...
## [](https://nickel-lang.org/user-manual/package-management/#other-combinators)Other combinators
...
## [](https://nickel-lang.org/user-manual/package-management/#blame-locations)Blame locations

Nickel tries to associate each contract violation with the location of the code that triggered it; a good blame location makes it much easier to find and fix the problem. When you use delayed contracts, Nickel's built-in laziness automatically pushes the error locations "inwards": in the following example, even though we annotate the entire record with a contract, Nickel helpfully points out that the error is caused by the string "8080" instead of blaming the whole record:
...
The narrowed-down blame location is a consequence of how delayed contracts work: the contract annotation `{ server_port = "8080" } | { server_port | Number }` evaluates via delayed contract application to `{ server_port | Number = "8080" }`. When the `server_port` field is eventually evaluated, the `Number` contract fails and blames the string "8080".

When custom contracts fail immediately, there is no inner lazy value to blame. If we were to write an immediate version of the `Schema` contract above, it would provide a less localized error that blames the whole record instead of just the problematic field:
...
To provide better blame locations for immediate contract failures, the error data returned by a custom contract supports an optional `blame_location` field, that you can fill out to specify the location that gets blamed. The easiest and most common way to take advantage of this is to use `std.contract.check` for checking inner values: `std.contract.check` automatically fills out `blame_location` on error, and so you will get the more specific error by default.
...
Most of the time the automatic propagation of `blame_location` should do what you want, as long as you use `std.contract.check` for checking inner values. For specific purposes, you can manipulate the blame location programmatically. For example, here is a contract for a record with two small numbers in it. If both numbers are too big, it blames whichever is bigger.
